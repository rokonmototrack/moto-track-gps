var app = {
    ...
    setupWysihtml5: function () {
        if (!$.fn.wysihtml5 || !$.fn.wysihtml5.defaultOptions) return;

        const parseSmsContent = (value, isSourceView) => {
            return isSourceView ? value.replace(/(<br>)/g, '\\r\\n') : value.replace(/(\\r\\n)/g, '<br>');
        }

        $.fn.wysihtml5.defaultOptions = Object.assign($.fn.wysihtml5.defaultOptions, {
            events: {
                load: function () {
                    if (!this.config.useSmsParser) return;
                    this.setValue(parseSmsContent(this.getValue()), false);
                },
                change_view: function() {
                    if (!this.config.useSmsParser) return;
                    this.setValue(parseSmsContent(this.getValue(), typeof this.currentView.selection !== 'undefined'));
                }
            },
            parserRules: {
                tags: Object.assign($.fn.wysihtml5.defaultOptions.parserRules.tags, {
                    table: {},
                    thead: {},
                    tbody: {},
                    th: {},
...
$(window).on('load', function(){
    ...
    $(document)
        .on('shown.bs.collapse', '#routes_tab .group-collapse', function () {
            dd('#routes_tab .group-title shown');
            app.routes.toggleGroup( $(this).attr('data-id') );
        })
        .on('hidden.bs.collapse', '#routes_tab .group-collapse', function () {
            dd('#routes_tab .group-title hidden');
            app.routes.toggleGroup( $(this).attr('data-id') );
        });
    ...
    $(document).on('focus', '.empty-input-add-new', function() {
        ...
        copy.find('.bootstrap-select').replaceWith(function() { return $('select', this); });
...
$(document).on('submit', '.confirmed-delete-form-btn', function(e) {
    e.preventDefault();

    let form = $(this);
    let loaderParent = form.data('loader-parent');

    if (!loaderParent) {
        loaderParent = '.modal-content'
    }

    loaderParent = $(this).closest(loaderParent);
    ...
    $.ajax({
        ...
        beforeSend: function () {
            if (loaderParent.length) {
                loader.add(loaderParent);
            }
        },
        complete: function () {
            if (loaderParent.length) {
                loader.remove(loaderParent);
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            handlerFail(jqXHR, textStatus, errorThrown);
